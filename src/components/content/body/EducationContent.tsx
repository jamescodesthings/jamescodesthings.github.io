  ...
  <article className={`${spacing.mb}`}>
    ...
    {entry.qualifications.map((q, i) => (
      <div key={`q-${i}`} className={`${styles.rowCollapse} !justify-start !items-baseline !mb-0`}>
        <h3 className={`${styles.summary} ${typography.h3} !font-bold !not-italic mr-2 !mb-0`}>{q.title}</h3>
        {q.aside && <h3 className={`${styles.summary} ${typography.leader} !text-base !mb-0`}>{q.aside}</h3>}
      </div>
    ))}
  </article>
);
